...
def everything_for_your_cat(cats_data: list[tuple[str, int, str, str]]) -> str:
    """
    Функция оптимизирует хранение данных таким образом, что для каждого владельца данные о всех его
    питомцы отображались в одной строке
    :param cats_data: Список кортежей с информацией о питомце и владельце
    :return: "многострочная" строка вида: Имя владельца Фамилия владельца: кличка питомца \n
    ...
    for tpl in cats_data:
        owner = f'{tpl[2]} {tpl[3]}'
        owners[owner] = owners.get(owner, '') + f'{tpl[0]}, {tpl[1]}; '
