...
class RomanNums:
    ...
    def from_roman(self):
        ...
        arab_num = 0
        # В цикле перебираем римскую строку, если текущая цифра больше чем следующая - прибавляем, иначе - вычитаем ее
        for idx in range(len(self.roman_string) - 1):
            if roman_dict[self.roman_string[idx]] < roman_dict[self.roman_string[idx + 1]]:
                arab_num -= roman_dict[self.roman_string[idx]]
            else:
                arab_num += roman_dict[self.roman_string[idx]]
    ...
    def is_palindrome(self):
        """
        Метод класса определяет является ли арабское число палиндромом
